...
{
public:
	AsyncServerCall(const std::shared_ptr<tie::InferenceService::AsyncService>& service, const std::shared_ptr<grpc::ServerCompletionQueue>& cq, const std::shared_ptr<engine::engine_interface>& engine_ptr)
		: _call_state{CallState::CREATE}, _service{service}, _cq{cq}, _engine_ptr{engine_ptr} {}
...
{
	explicit AsyncServerCallIO() : rpc{ &context } { }
	RequestT request;
	ResponseT response;
...
{
public:
	explicit EngineReadyAsyncCall(const std::shared_ptr<tie::InferenceService::AsyncService>& service, const std::shared_ptr<grpc::ServerCompletionQueue>& cq, const std::shared_ptr<engine::engine_interface>& engine_ptr)
		: AsyncServerCall(service, cq, engine_ptr), _io{ AsyncServerCallIO<tie::EngineReadyRequest, tie::EngineReadyResponse>() }
	{
	...
	~EngineReadyAsyncCall()
	{
		spdlog::trace("deleting EngineReadyAsyncCall");
	}
	...
	bool process(bool ok) override
	{
		...
		switch (_call_state)
		{
			...
			default:
				spdlog::error("EngineReadyAsyncCall - Unexpected tag: {}", int(_call_state));
				assert(false);
		}
...
{
public:
	explicit ModelReadyAsyncCall(const std::shared_ptr<tie::InferenceService::AsyncService>& service, const std::shared_ptr<grpc::ServerCompletionQueue>& cq, const std::shared_ptr<engine::engine_interface>& engine_ptr)
		: AsyncServerCall(service, cq, engine_ptr), _io{ AsyncServerCallIO<tie::ModelReadyRequest, tie::ModelReadyResponse>() }
	{
	...
	~ModelReadyAsyncCall()
	{
		spdlog::trace("deleting ModelReadyAsyncCall");
	}
	...
	bool process(bool ok) override
	{
		...
		switch (_call_state)
		{
			...
			default:
				spdlog::error("ModelReadyAsyncCall - Unexpected tag: {}", int(_call_state));
				assert(false);
		}
...
{
public:
	explicit LoadModelAsyncCall(const std::shared_ptr<tie::InferenceService::AsyncService>& service, const std::shared_ptr<grpc::ServerCompletionQueue>& cq, const std::shared_ptr<engine::engine_interface>& engine_ptr)
		: AsyncServerCall(service, cq, engine_ptr), _io{ AsyncServerCallIO<tie::ModelLoadRequest, tie::ModelLoadResponse>() }
	{
	...
	~LoadModelAsyncCall()
	{
		spdlog::trace("deleting LoadModelAsyncCall");
	}
	...
	bool process(bool ok) override
	{
		...
		switch (_call_state)
		{
			...
			default:
				spdlog::error("LoadModelAsyncCall - Unexpected tag: {}", int(_call_state));
				assert(false);
		}
...
{
public:
	explicit UnloadModelAsyncCall(const std::shared_ptr<tie::InferenceService::AsyncService>& service, const std::shared_ptr<grpc::ServerCompletionQueue>& cq, const std::shared_ptr<engine::engine_interface>& engine_ptr)
		: AsyncServerCall(service, cq, engine_ptr), _io{ AsyncServerCallIO<tie::UnloadModelRequest, tie::UnloadModelResponse>() }
	{
	...
	~UnloadModelAsyncCall()
	{
		spdlog::trace("deleting UnloadModelAsyncCall");
	}
	...
	bool process(bool ok) override
	{
		...
		switch (_call_state)
		{
			...
			default:
				spdlog::error("UnloadModelAsyncCall - Unexpected tag: {}", int(_call_state));
				assert(false);
		}
